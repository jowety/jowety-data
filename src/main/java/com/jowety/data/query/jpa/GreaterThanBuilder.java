...
public class GreaterThanBuilder  extends PredicateBuilderBase {

	public GreaterThanBuilder(boolean negative) {
		super(negative);
	}
	...
	public <X> Predicate build(CriteriaBuilder cb, Root<X> from, Filter<X> filter, Map<String, Path> pathMap) {
		...
		Predicate out = isNegative(filter.isNegative())?
				cb.not(cb.greaterThan(left, right)):
					cb.greaterThan(left, right);
				return out;
	}
